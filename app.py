...
with st.sidebar:
    ...
    # Priority 2: Use the Text Input box
...
if st.button("🚀 Generate 10 Pro Leads"):
    if not api_key_input:
        st.error("❌ API Key is missing! Please paste it in the sidebar.")
    elif not my_company_desc:
        st.warning("⚠️ Please fill in your Company Profile first.")
    else:
        try:
            # --- THE CRITICAL FIX ---
            # We "Force" the configuration right here inside the button
            ...
            with st.spinner("🔍 Mining 10 leads..."):
                ...
                if not response.text:
                    ...
                    st.download_button("📥 Download Results", data=response.text, file_name="samketan_leads.csv")
