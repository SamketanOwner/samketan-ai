...
def get_engine(key):
    try:
        ...
        # Check for Gemini 1.5 Flash (highest free quota: 1500/day)
...
if st.button("🚀 Generate & View Full Leads"):
    if not api_key:
        ...
        if model:
            with st.spinner("🔍 Mining detailed leads..."):
                prompt = f"""
                ...
                - Identify a likely 'Decision Maker Role' (e.g., F&B Manager, Purchase Head).
                ...
                try:
                    ...
                    if response:
                        ...
                        html_table += "<tr style='background-color: #004a99; color: white;'><th>Business Details</th><th>Website</th><th>Email (Compose)</th><th>WhatsApp (Shoot)</th><th>LinkedIn Connection</th></tr>"
                        ...
                        for line in lines:
                            if '|' in line and 'Agency' not in line and '---' not in line:
                                ...
                                # --- DIRECT LINKEDIN PEOPLE SEARCH FIX ---
                                # This forces LinkedIn to search in the 'People' category for Name + Company
                                li_search_query = f'"{person}" "{name}"'
                                li_link = f"https://www.linkedin.com/search/results/people/?keywords={urllib.parse.quote(li_search_query)}"
                                ...
                                <tr>
                                    ...
                                    <td style='border: 1px solid #ddd; padding: 10px;'>
                                        ...
                                        <a href='{li_link}' target='_blank' style='color: #0a66c2; font-weight: bold;'>🔗 Direct Profile Search</a>
                                    </td>
                                </tr>"""
                    ...
                    if "429" in str(e):
                        st.warning("⚠️ Daily limit reached. Please wait 60 seconds or use a fresh API key.")
                    else:
                        st.error(f"API Error: {e}")

# --- FOOTER ---
st.markdown("---")
st.caption("Samketan Engine v3.7 | LinkedIn People-Search Enabled")
