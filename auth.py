...
# --- CHECK STREAMLIT BUILT-IN OAUTH ---
def check_builtin_google_auth():
    """
    Uses Streamlit's built-in OAuth (v1.35+).
...
def inject_css(logo_b64=None):
    ...
    div[data-testid="stTextInput"] > div > div > input {{
        ...
        border-radius: 9px !important; border: 1px solid #d0d7e8 !important;
        padding: 10px 14px !important; font-size: 14px !important;
        background: #f8f9fc !important; color: #0D1B3E !important;
    }}
    ...
    div[data-testid="stSuccess"] {{ border-radius: 9px !important; font-family: 'DM Sans', sans-serif !important; font-size: 13px !important; }}
    div[data-testid="stError"] {{ border-radius: 9px !important; font-family: 'DM Sans', sans-serif !important; font-size: 13px !important; }}
    </style>
...
def login_screen():
    ...
    # ── Check Streamlit built-in OAuth (no extra library) ──
    try:
        user = st.experimental_user
    ...
    with col_right:
        ...
        if not st.session_state.get("otp_sent"):
            ...
            # Try Streamlit built-in st.login first
            google_available = False
            try:
                if hasattr(st, "login") and "auth" in st.secrets:
                    google_available = True
            except Exception:
                pass

            if google_available:
                # Streamlit native OAuth button (v1.40+)
                st.login("google")
            else:
                # Styled non-functional placeholder — shows setup needed
                st.markdown(f"""
                <div style="display:flex;align-items:center;justify-content:center;gap:8px;
                    background:#fff;border:1px solid #dadce0;border-radius:9px;
                    padding:11px 16px;font-family:'DM Sans',sans-serif;
                    font-size:14px;font-weight:500;color:#3c4043;margin-bottom:4px;">
                  {google_icon} Continue with Google
                </div>
                """, unsafe_allow_html=True)
                
                # Try streamlit-google-auth as fallback
                try:
                    ...
                    if "google_oauth" in st.secrets:
                        ...
                        auth = Authenticate(
                            ...
                            cookie_key=cfg.get("cookie_key", "samketan_key_2026"),
                            redirect_uri=cfg.get("redirect_uri", "https://samketanai.streamlit.app"),
                        ...
                        if st.session_state.get("connected"):
                            ...
                            log_to_google_sheet(email, "Google OAuth")
                            st.session_state.authenticated = True
                    ...
                    st.markdown(f'<p style="font-family:\'DM Sans\',sans-serif;font-size:11px;color:#e06060;text-align:center;margin:2px 0 6px;">Google Sign-In error: {e}</p>', unsafe_allow_html=True)
